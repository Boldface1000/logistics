...
export function SupportChat() {
  ...
  useEffect(() => {
    ...
    const fetchMsgs = async () => {
      ...
      if (!open) {
        setUnread(data.filter((m) => m.sender_is_admin && !messages.find((x) => x.id === m.id)).length);
      }
    };
    fetchMsgs();
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-sm p-0 gap-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 bg-primary text-primary-foreground">
            ...
            <button onClick={() => setOpen(false)} aria-label="Close" className="h-8 w-8 rounded-full bg-white/15 flex items-center justify-center">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div ref={scrollRef} className="h-80 overflow-y-auto p-3 space-y-2 bg-background">
            {messages.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-8">Send us a message — we'll get back to you shortly.</p>
            )}
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.sender_is_admin ? "justify-start" : "justify-end"}`}>
                <div className={`max-w-[75%] px-3 py-2 rounded-2xl text-sm ${
                  m.sender_is_admin ? "bg-secondary text-secondary-foreground" : "bg-primary text-primary-foreground"
                }`}>
                  {m.body}
                </div>
              </div>
